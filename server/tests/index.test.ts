...
beforeAll(async () => {
  ...
  const res = await axios.post('http://localhost:3001/login', {username: 'test', password: 'test'});
...
describe('GET /pathwayTemplates', () => {
  test('responds with all pathway templates', async () => {
    const response = await request(app).get('/pathwayTemplates').set('Authorization', 'Bearer ' + token);
    expect(response.statusCode).toBe(200);
...
describe('POST /pathwayTemplates', () => {
  test('responds with the newly created pathway template', async () => {
    ...
    const response = await request(app)
      .post('/pathwayTemplates').set('Authorization', 'Bearer ' + token)
      .send(newProcedure);
    expect(response.statusCode).toBe(200);
...
describe('PUT /pathwayTemplates/:id', () => {
  test('responds with the id of the updated pathway template', async () => {
    ...
    const response = await request(app)
      .put('/pathwayTemplates/test-id').set('Authorization', 'Bearer ' + token)
      .send(updatedProcedure);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual('test-id');

    const template = await request(app).get('/pathwayTemplates/test-id').set('Authorization', 'Bearer ' + token);
...
describe('DELETE /pathwayTemplates/:id', () => {
  test('responds with the id of the deleted pathway template', async () => {
    const response = await request(app).delete('/pathwayTemplates/test-id').set('Authorization', 'Bearer ' + token);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual('test-id');

    const all = await request(app).get('/pathwayTemplates').set('Authorization', 'Bearer ' + token);
...
describe('GET /stageTemplates', () => {
  test('responds with all stage templates', async () => {
    const response = await request(app).get('/stageTemplates').set('Authorization', 'Bearer ' + token);
    expect(response.statusCode).toBe(200);
...
describe('POST /stageTemplates', () => {
  test('responds with the newly created stage template', async () => {
    ...
    const response = await request(app).post('/stageTemplates').set('Authorization', 'Bearer ' + token).send(newStage);
    expect(response.statusCode).toBe(200);
...
describe('PUT /stageTemplates/:id', () => {
  test('responds with the id of the updated stage template', async () => {
    ...
    const response = await request(app)
      .put('/stageTemplates/test-stage-id').set('Authorization', 'Bearer ' + token)
      .send(updatedStage);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual('test-stage-id');

    const template = await request(app).get('/stageTemplates/test-stage-id').set('Authorization', 'Bearer ' + token);
...
describe('DELETE /stageTemplates/:id', () => {
  test('responds with the id of the deleted stage template', async () => {
    const response = await request(app).delete('/stageTemplates/test-stage-id').set('Authorization', 'Bearer ' + token);
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual('test-stage-id');

    const all = await request(app).get('/stageTemplates').set('Authorization', 'Bearer ' + token);
